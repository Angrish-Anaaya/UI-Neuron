...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 it it.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "T-type calcium current";
...
static int  rates (  double _lv ) {
   minf = 1.0 / ( 1.0 + exp ( - ( _lv + 59.0 ) / 6.2 ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv + 83.0 ) / 4.0 ) ) ;
   taum = 0.612 + 1.0 / ( exp ( - ( _lv + 132.0 ) / 16.7 ) + exp ( ( _lv + 16.8 ) / 18.2 ) ) ;
   tauh = exp ( ( _lv + 467.0 ) / 66.6 ) ;
   if ( _lv > - 81.0 ) {
     tauh = 28.3 + exp ( - ( _lv + 12.6 ) / 11.1 ) ;
     }
    return 0; }
 ...
 v=_v;
{
  eca = _ion_eca;
 { error =  states();
 if(error){fprintf(stderr,"at line 38 in file it.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
